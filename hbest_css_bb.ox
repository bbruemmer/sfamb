...
/*Data preparation*/	
	...
	fob.Renew((fob.GetVar("trend")) .* fob.GetVar("ID"),"tID");
	fob.Renew((fob.GetVar("trend").^2) .* fob.GetVar("ID"),"t2ID");

	decl mTmp, asTmp, vN, cN, i, vID = fob.GetVar("ID"), vT = fob.GetVar("trend");
	vN = unique(vID)';
	cN = sizer(vN);
	mTmp = zeros(sizeof(vID),cN); // matrix for ID dummies
	asTmp = new array[3][cN];					  // names for Variables  D,DT,DT2
	for (i = 0; i < cN; ++i){
		mTmp[][i] = (vID .== vN[i]);
		asTmp[0][i] = sprint("D",i+1);
		asTmp[1][i] = sprint("DT",i+1);
		asTmp[2][i] = sprint("DTT",i+1);
		}
	print(asTmp);
	fob.Renew(mTmp,asTmp[0]);
	fob.Renew(mTmp.*vT,asTmp[1]);
	fob.Renew(mTmp.*(vT.^2),asTmp[2]);
